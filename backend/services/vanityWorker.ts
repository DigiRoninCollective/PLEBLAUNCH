// vanityWorker.ts - Worker thread for vanity address generation
import { parentPort, workerData } from 'worker_threads';
import { Keypair } from '@solana/web3.js';
import * as bip39 from 'bip39';
import { derivePath } from 'ed25519-hd-key';

const { pattern, type, jobId } = workerData as { pattern: string; type: string; jobId: string };
...
async function generateVanityWallet(): Promise<void> {
  let attempts = 0;
