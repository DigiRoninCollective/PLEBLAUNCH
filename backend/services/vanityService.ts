...
class VanityService {
    ...
    calculateDifficulty(pattern: string, type: string): Difficulty {
    ...
    getDifficultyWarning(estimatedSeconds: number): string {
    ...
    validatePattern(pattern: string, type: string) {
        const errors: string[] = [];
        ...
        return {
            ...
            errors,
            difficulty: difficulty // Fallback to difficulty object to avoid TypeScript error
        };
    }

    // Start vanity generation
    async startGeneration(userId: number, pattern: string, type: string, telegramBot: any) {
    ...
    handleWorkerMessage(userId: number, message: any, telegramBot: any) {
    ...
    handleProgress(userId: number, message: any, telegramBot: any) {
        const jobInfo = this.activeJobs.get(userId);
        if (!jobInfo) return;
    ...
    handleSuccess(userId: number, message: any, telegramBot: any) {
        const jobInfo = this.activeJobs.get(userId);
        if (!jobInfo) return;
    ...
    handleWorkerError(userId: number, error: Error, telegramBot: any) {
    ...
    // Cancel generation
    cancelGeneration(userId: number, reason = 'Cancelled by user', telegramBot: any) {
        const jobInfo = this.activeJobs.get(userId);
        if (!jobInfo) {
            return false;
        }
    ...
    // Get start message
    getStartMessage(pattern: string, type: string, difficulty: Difficulty) {
        const warningEmojis = {
            'easy': '🟢',
            'medium': '🟡', 
            'hard': '🟠',
